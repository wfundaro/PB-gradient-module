...
Module gradient
  ...
  Procedure init_transparent_background_image()
    ...
    StartDrawing(ImageOutput(#transparent_background_image))
    pas = 7
    For y = 0 To (#SIZE_WORK_IMAGE / pas)
      For x = 0 To (#SIZE_WORK_IMAGE / pas)
        ...
        Else
          bcolor = RGB(180,180,180)
        EndIf
        Box(x * pas, y * pas, pas, pas, bcolor)
        invert = 1!invert
      Next x
      ;invert = 1!invert
    Next y  
  ...
  Procedure new(hwnd, gadget, posx, posy, sx, sy)         ;creating gradient gadget
    Protected ret.i
    If IsImage(#transparent_background_image) = 0
  ...
  Procedure draw(gadget)
    ...
    For y = 1 To sy - 31
      ...
      invert = 1!invert
    Next
;     ClipOutput(6, 1, sx - 12, sy - 32) 
;     DrawImage(ImageID(#transparent_background_image), 6, 1)
;     UnclipOutput() 
